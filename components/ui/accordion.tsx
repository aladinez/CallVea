...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-zinc-800/80 py-2", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-left font-medium text-zinc-100 transition-all hover:text-indigo-400 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
